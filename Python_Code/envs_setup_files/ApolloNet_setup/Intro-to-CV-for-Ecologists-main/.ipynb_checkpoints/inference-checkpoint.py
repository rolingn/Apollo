## The code that runs the custom trained inference model 'ApolloNet'. 
## Classifies the cropped images into one of the following 5 classes: "fly", "moth", "no_insect", "other", "schnake"
## Saves a results csv with the columns: 'Filename', 'Year', 'Month', 'Day', 'Hour', 'Minute', 'Second', 'Best_Class', 'Best_Score'
## The results csv is saved in the results folder of each day-folder in the Diopsis data structure. 
...
## Fetches local environment path and searches for the most recent model in the environment "ApolloNet" in the subfolders "Intro-to-CV-for-Ecologists-main" and "model_weights".
...
most_recent_model = fetch_most_recent_model(os.path.join(envs_path, "ApolloNet", "Intro-to-CV-for-Ecologists-main", "model_weights"))
...
## Also saves the results into the results folder of the specific day as "ApolloNet_classification_results.csv"
def predict(folder):
    images = os.listdir(folder)
    images = sorted(images)
    class_names = ["fly", "moth", "no_insect", "other", "schnake"]
    results = []
    ...
    for img in images:
        ...
        score = preds[0, predicted_index]
        results.append([img, year, month, day, hour, minute, second, pred_class, score])

    main_dir = reduce_path(folder, 1)
    classification_folder = os.path.join(main_dir, "results")
    os.makedirs(classification_folder, exist_ok=True)
    csv_file_path = os.path.join(classification_folder, "ApolloNet_classification_results.csv")
    with open(csv_file_path, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Filename', 'Year', 'Month', 'Day', 'Hour', 'Minute', 'Second', 'Best_Class', 'Best_Score'])  # Write header
