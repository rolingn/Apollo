...
JATZ_low <- read_smet("Jatz_1.smet")$data %>%
  ...
  mutate(
    ...
    Hour = hour(timestamp),
    DayNight = if_else(Hour >= 5 & Hour < 21, "Day", "Night"),
    Date = if_else(Hour < 5, as.Date(timestamp) - 1, as.Date(timestamp))  # Assign night hours before 5AM to previous day
  ) %>%
  ...
  filter(!is.na(Date) & !is.na(DayNight)) %>%
  mutate(
    timestamp = case_when(
      DayNight == "Day" ~ ymd_hms(paste(Date, "05:00:00")),
      DayNight == "Night" ~ ymd_hms(paste(Date, "21:00:00"))
    )
  ) %>%
...
setwd("/Volumes/Apollo/Davos/Data/CSV_tables/smet_as_csv_daily/200mGrid")
...
WFJ_low <- read_smet("SLF2.smet")$data %>%
  convert_kelvin_to_C() %>%
  mutate(
    PSUM = if_else(PSUM == -999.0, 0, PSUM),
    timestamp = ymd_hms(timestamp),
    Hour = hour(timestamp),
    DayNight = if_else(Hour >= 5 & Hour < 21, "Day", "Night"),
    Date = if_else(Hour < 5, as.Date(timestamp) - 1, as.Date(timestamp))  # Assign night hours before 5AM to previous day
  ) %>%
  select(-DW, -HS, -TSS, -VW_MAX, -Hour) %>%
  group_by(Date, DayNight) %>%
...
setwd("/Volumes/Apollo/Davos/Data/CSV_tables/smet_as_csv_daily/preliminary")
