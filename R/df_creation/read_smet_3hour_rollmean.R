...
JATZ_low <- read_smet("Jatz_1.smet")$data %>%
  ...
  filter(!is.na(time_window)) %>%  # Ensure no NA in index
  group_by(time_window) %>%
  summarise(across(where(is.numeric), mean, na.rm = TRUE), .groups = "drop") %>%
  arrange(time_window) %>%
  mutate(across(where(is.numeric),
                ~ slide_index_dbl(.x, .i = time_window, .f = mean, .before = days(3), .complete = TRUE))) %>%
  filter(time_window >= start_date & time_window <= end_date) %>%
...
setwd("/Volumes/Apollo/Davos/Data/CSV_tables/smet_as_csv_3hour_rollmean3d")
