### Script for gathering all the images classified as dirt by the dirt detector from all the different folders
    ...
    for folder in os.listdir(photos_path):
        ...
        if os.path.isdir(folder_path):
            ...
            if os.path.isdir(cropped_path):
                # Path to the 'potentially_faulty' folder
                faulty_path = os.path.join(cropped_path, 'potentionally_faulty')
                #print(f"Jetzt in {faulty_path}.")

                if os.path.isdir(faulty_path):
                    # Copy all images from 'potentially_faulty' to the destination folder
                    for file in os.listdir(faulty_path):
                        source_file = os.path.join(faulty_path, file)
